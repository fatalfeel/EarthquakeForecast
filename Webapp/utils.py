...
#app = Flask(__name__)
def prepare_earthquake_data_and_model(days_out_to_predict = 7, max_depth=100, eta=0.00001):
    '''
...
def get_earth_quake_estimates(desired_date, df_live):
    ...
    live_set_tmp = df_live[df_live['date'].dt.strftime('%Y-%m-%d') == desired_date]
...
def build_page():
        if request.method == 'POST':
            ...
            return render_template('index.html',
                ...
                earthquake_horizon = get_earth_quake_estimates( str(horizon_date)[:10],
                                                                earthquake_live),
